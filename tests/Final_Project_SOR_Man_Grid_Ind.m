...
z1=9;                           %Number of times the code will double
...
%w=1.9;                         %If one wants to try another relax const
w_1=(1-w);                      %Constant needed for SOR loop

%Constants that will be used inside the SOR loop
...
Uaverage(z,:)=sum(sum(U))/((Nx+2)*(Ny+2));
end

T=table(TwoNx,Xnodes,Ynodes,InnerloopIterations,ReferenceError,L2Error,logL2Error,logDeltaX,Uaverage);

figure()
plot(logDeltaX,logL2Error);
xlabel('log(\Delta x)')
ylabel('log(L2 Error)')
title('logL2Error vs Log(\Delta x)');
