...
if exist('checkpt_SOR_1st_Order_h.mat','file')     %If a checkpoint file exists, open it
    load('checkpt_SOR_1st_Order_h.mat')
end
...
while err1>tol
    for i=2:Nx+1
        ...
        %Neumann Condition
        U1(i,Ny+2)=(w_1)*U1(i,Ny+2)+w*((dx2dy2*F1(i,Ny+2)-dy2*U1(i-1,Ny+2)-dy2*U1(i+1,Ny+2)-dx2*U1(i,Ny+2)-dx2*U1(i,Ny+1))/(denomin));
    end
    err1=max(max(abs(Uprev1-U1)./abs(Uprev1)));
    Uprev1=U1;
    iter1=iter1+1;
    if mod(iter1,100)==0                               %Save checkpoint file every 1500 iterations
        save('checkpt_SOR_1st_Order_h.mat');                   %Saving the file
    end                                                 %Ending if loop
end
...
zlabel('U values');
title('Surface Plot for SOR (Nx=Ny=60) w/1st Order Neumann');
colorbar;
...
set(h1,'linestyle','none')
zlabel('U values');
title('Contour Plot for SOR (Nx=Ny=60) w/1st Order Neumann');
colorbar;

%%
delete('checkpt_SOR_1st_Order_h.mat');             %Delete checkpoint file once complete