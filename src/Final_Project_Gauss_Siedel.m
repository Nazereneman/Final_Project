...
zlabel('U values');
title('Surface Plot for GS (Nx=Ny=60)');
colorbar;
...
set(h1,'linestyle','none')
zlabel('U values');
title('Contour Plot for GS (Nx=Ny=60)');
colorbar;
...
while err2>tol
    for i=2:Ny+1
        for j=2:Nx+1
            U2(i,j)=(dx2dy2*F2(i,j)-dy2*U2(i-1,j)-dy2*U2(i+1,j)-dx2*U2(i,j-1)-dx2*U2(i,j+1))/(denomin);
        end
        U2(i,Ny+2)=(dx2dy2*F2(i,Ny+2)-dy2*U2(i-1,Ny+2)-dy2*U2(i+1,Ny+2)-dx2*U2(i,Ny+1)-dx2*U2(i,Ny+1))/(denomin);
    end
...
zlabel('U values');
title('Surface Plot for GS (F=0, Nx=Ny=60)');
colorbar;
...
set(h2,'linestyle','none')
zlabel('U values');
title('Contour Plot for GS (F=0, Nx=Ny=60)');
colorbar;
%%
